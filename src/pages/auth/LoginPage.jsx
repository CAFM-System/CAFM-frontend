...
export default function LoginPage() {
  ...
  const theme = {
    bgOverlay: isDarkMode 
      ? "bg-gradient-to-br from-[#18181B] via-[#27272A]/95 to-[#1F1F23]"
      : "bg-gradient-to-br from-[#FFFFFF]/95 via-white/90 to-[#F9FAFB]/90",
    card: isDarkMode 
      ? "bg-[#1F1F23]/80 border-[#3F3F46]" 
      : "bg-white/80 border-[#D1D5DB]",
    textPrimary: isDarkMode ? "text-[#F5F5F5]" : "text-[#18181B]",
    textSecondary: isDarkMode ? "text-[#A1A1AA]" : "text-[#4B5563]",
    textMuted: isDarkMode ? "text-[#A1A1AA]/70" : "text-[#6B7280]",
    input: isDarkMode 
      ? "bg-[#27272A] text-[#F5F5F5] placeholder-[#A1A1AA] border-[#3F3F46]" 
      : "bg-[#F3F4F6] text-[#18181B] placeholder-gray-400 border-[#D1D5DB]",
    buttonPrimary: "w-full bg-accent hover:bg-[#FBBF24] text-secondary font-bold py-2.5 rounded-xl shadow-xl hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 border border-accent/50"
  };
    ...
    <div className="flex items-center justify-center min-h-screen relative overflow-hidden">
      ...
      <button 
        ...
        className={`absolute top-6 right-6 z-50 p-3 rounded-full backdrop-blur-sm transition-all duration-300 border shadow-lg ${
          isDarkMode 
            ? "bg-[#F5F5F5]/10 hover:bg-[#F5F5F5]/20 text-[#F5F5F5] border-[#F5F5F5]/20"
            : "bg-[#18181B]/10 hover:bg-[#18181B]/20 text-[#18181B] border-[#18181B]/20"
        }`}
        title="Toggle Theme"
      >
          ...
          <div className="space-y-3 pt-4">
            {[{icon: Sparkles, text: "Real-time Tracking"}, {icon: TrendingUp, text: "Enhanced Productivity"}, {icon: CheckCircle2, text: "Seamless Integration"}].map((feature, i) => (
              <div key={i} className={`flex items-center gap-3 p-4 rounded-xl border backdrop-blur-md transition-colors duration-300 ${
                isDarkMode 
                  ? "bg-[#F5F5F5]/5 border-[#F5F5F5]/10 hover:bg-[#F5F5F5]/10"
          ...
          <div className={`w-full rounded-3xl shadow-2xl p-6 border transition-all duration-500 ${theme.card}`}>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${isDarkMode?"text-[#F5F5F5]/60 hover:text-accent":"text-gray-500 hover:text-accent"}`}>
            ...
            <div className="mb-4 text-right">
              <button onClick={handleForgotPassword} className={`text-sm font-medium transition-colors duration-300 hover:underline ${isDarkMode?"text-accent hover:text-[#F5F5F5]":"text-accent hover:text-[#18181B]"}`}>
                Forgot Password?
              </button>
            </div>
            ...
            <div className="relative my-4">
              <div className="absolute inset-0 flex items-center">
                <div className={`w-full border-t ${isDarkMode?"border-[#A1A1AA]/20":"border-gray-200"}`}></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className={`px-4 backdrop-blur-sm rounded-full font-medium ${isDarkMode?"bg-[#18181B]/40 text-[#A1A1AA]/60":"bg-white/60 text-gray-500"}`}>
                  New to FACILITRON?
                </span>
              </div>
            </div>
            ...
            {/* Register */}
            <button onClick={handleCreateAccount} className={`w-full font-semibold py-2.5 rounded-xl backdrop-blur-sm border transition-all duration-300 shadow-lg ${isDarkMode?"bg-[#F5F5F5]/5 hover:bg-[#F5F5F5]/10 text-[#F5F5F5] border-[#A1A1AA]/20":"bg-[#F3F4F6]/5 hover:bg-[#F3F4F6]/10 text-[#18181B] border-[#D1D5DB]"}`}>
              Create Account
            </button>
